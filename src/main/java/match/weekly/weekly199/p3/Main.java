...
class Solution {
    ...
    private Node dfs(TreeNode root, int distance) {
        ...
        Node left = dfs(root.left, distance);
        Node right = dfs(root.right, distance);
        Node current = new Node();
        current.count = left.count + right.count;
        for (Map.Entry<TreeNode, Integer> a : left.dis.entrySet()) {
            for (Map.Entry<TreeNode, Integer> b : right.dis.entrySet()) {
                if (a.getValue() + b.getValue() <= distance) current.count += 1;
            }
        }
        for (Map.Entry<TreeNode, Integer> a : left.dis.entrySet()) {
